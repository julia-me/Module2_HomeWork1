...
const createPizza = function(pizzaObj) {
  ...
  btnElement.onclick = function(){
      const index = productsArr.findIndex(pizza => pizza.id == btnElement.dataset.id)
...
const renderPizza = () => {
  ...
  infoContainer.innerHTML =` 
                              ...
                              `      
                              importantInfo.price = +document.querySelector('.cart__total-price').dataset.price
                              importantInfo.totalAmount = +document.querySelector('.cart__total-amount').dataset.count
    ...
    promoBtn.addEventListener('click', ()=> {
        if(promoText.value.toLowerCase() == 'anton' && importantInfo.discont === false || importantInfo.discont == discont){
            ...
            totalPriceText.innerText = `Итого к оплате: ${Math.ceil(totalPrice * discont)} грн.`
